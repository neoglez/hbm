...
class OSmplWithPose:    
    ...
    def __init__(self, mean_template_shape, shape_blendshapes, 
                 pose_blendshapes, shape = None, pose = None):
        assert isinstance(mean_template_shape, OSmplTemplate), (
    ...
    def apply_pose_blend_shapes(self, thetas):
        ...
        if thetas is not None:
            ...
            self.vertices_posed = (self.vertices_shaped + 
                self.pose_blendshapes.dot(posemap_tmp)
                )
            self.is_posed = True
            self.must_recompute_skeleton = True
    ...
    def calculate_stature_from_mesh(self):
        ...
        stature = np.linalg.norm(
            np.subtract(vertex_with_biggest_y.view(dtype='f8'),
    ...
    def calculate_shoulder_length_from_mesh(self):
        ...
        # left shoulders, then calculate the euclidean distance between them.
        inverted_joint_names =  dict((v,k) 
                     for k,v in self.mean_template_shape.joint_names.items())
        
        shoulder_left_xyz = joints_location[
            inverted_joint_names['L_Shoulder']]
        shoulder_right_xyz = joints_location[
            inverted_joint_names['R_Shoulder']]
        shoulder_length = np.linalg.norm(
            np.subtract(shoulder_left_xyz.view(dtype='f8'),
    ...
    def write_annotations_to_file(self, annotations_path):
        with open( annotations_path, 'w') as fp:
            ...
            "human_dimensions": {
                ...
                "chest_circumference": self.dimensions.chest_circumference,
                "waist_circumference": self.dimensions.waist_circumference,
                "pelvis_circumference": self.dimensions.pelvis_circumference
                }
            },
    ...
    def read_annotations_from_file(self, annotations_path):
        ...
        self.dimensions = HumanDimensions(
                            ...
                            dimensions.get('shoulder_length'),
                            dimensions.get('hip_length'),
...
if __name__ == "__main__":
    ...
    ####################################################################
    #new_female_template = OSmplTemplate(female_model.get('template_mesh_vertices'),
                                        #female_model.get('template_mesh_faces'),
                                        #female_model.get('blend_weights'),
                                        #female_model.get('shape_blend_shapes'),
                                        #female_model.get('k_joints_predictor'), 
                                        #female_model.get('pose_blend_shapes'))
    ...
    ####################################################################
    human_female_model = OSmplWithPose(new_female_template,
                                       female_model.get('shapedirs').x,
                               female_model.get('posedirs'), 
                               None, None)
    human_male_model = OSmplWithPose(new_male_template,
