...
class OSmpl:
    def __init__(
        self, mean_template_shape, shape_blendshapes, shape=[], pose=[]
    ):
    ...
    def recompute_skeleton(self):
        # @todo handle case when the mesh DO has been posed
        joint_xyz = self.mean_template_shape.k_joint_predictor.regression_matrix.dot(
            self.vertices_shaped
        )
        return joint_xyz
    ...
    def apply_pose_blend_shapes(self, thetas):
        ...
        if thetas is not None:
            ...
            self.vertices_posed = (
                self.vertices_shaped + self.pose_blendshapes.dot(thetas)
            )
            self.must_recompute_skeleton = True
    ...
    def calculate_shoulder_length_from_mesh(self):
        ...
        inverted_joint_names = dict(
            (v, k) for k, v in self.mean_template_shape.joint_names.items()
        )
    ...
    def calculate_hip_length_from_mesh(self):
        ...
        hip_length = np.linalg.norm(
            np.subtract(
                hip_left_xyz.view(dtype="f8"), hip_right_xyz.view(dtype="f8")
            )
        )
...
if __name__ == "__main__":
    ...
    test_model = (
        "/home/neoglez/H-DIM-Project/dataset" "/male/male_mesh_009041.obj"
    )
